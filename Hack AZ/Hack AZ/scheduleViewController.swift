...
class ScheduleViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if rowPressed == 0{
            self.performSegue(withIdentifier: "ScheduleFridaySegue", sender: self)
        }else if rowPressed == 1 {
            self.performSegue(withIdentifier: "ScheduleSaturdaySegue", sender: self)
        }else if rowPressed == 2 {
            self.performSegue(withIdentifier: "ScheduleSundaySegue", sender: self)
        }
    }
